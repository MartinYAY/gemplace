...
function sanitiseChat(chat) {
    ...
    chat = chat.replace(badRegex, '[swearword]');
    // trim whitespace
        ...
        for (var name in this.ephemeralRooms) {
            if (this.ephemeralRooms.hasOwnProperty(name)) {
                list.push({
                    ...
                    thumbnail: '/gemplace/media/rooms/cave-thumb.png'
                });
            }
        }
        return list;
...
function handleCommand(cmd, myNick, user) {
            ...
            if (roomName.substr(0, 6) === 'house ') {
                ...
                if (User.hasAccount(houseName)) {
                    if (User.isHouseLocked(houseName) && myNick !== houseName) {
                        sendLine('This house is locked.');
                    } else {
                        doRoomChange(roomName, user);
                    }
                } else {
                    sendLine('The user "' + houseName + '" is homeless.');
                }
            } else {
    ...
    // mod help
        ...
        sendMultiLine([
                'Ten mod commands available: 1) kick, 2) kickban, 3) warn, 4) unban, 5) broadcast, 6) aliases, 7) move, 8) bits',
                ...
                "8. bits - Adds to or removes from someone's bits balance, e.g. /bits 20 flutterhorse, /bits -10 otherguy",
                'See also: /help'
    ...
    // kickbanning
        ...
        if (!User.has(kickee)) {
            sendLine('There is no user named "' + kickee + '"');
            return;
        }
        if (User.isModerator(kickee)) {
            sendLine('You cant kickban other moderators.');
            return;
        }
        ...
        User.forEach(function (iterUser) {
            if (iterUser.conn.remoteAddress === IP) {
                ...
                console.log('Banned "' + iterUser.nick + '" IP: ' + IP);
                sendLine('Banned "' + iterUser.nick + '" IP: ' + IP);
                aliases.push({
                    nick: iterUser.nick,
                    room: iterUser.room,
                    state: iterUser.obj
    ...
    // kicking
        ...
        User.forEach(function (iterUser) {
            if (iterUser.conn.remoteAddress === IP) {
                ...
                sendLine('Kicked "' + iterUser.nick + '" IP: ' + IP);
                aliases.push({
                    nick: iterUser.nick,
                    room: iterUser.room,
                    state: iterUser.obj
    ...
    // forced move
        ...
        if (pos !== -1) {
            ...
            if (!User.has(movee)) {
                sendLine('There is no user named "' + movee + '"');
                return;
            }
            if (User.isModerator(movee)) {
                sendLine('You cant move other moderators.');
                return;
            }
            modLogger.logMove(myNick, movee, User.get(movee).room, room, User.get(movee).obj);
            doRoomChange(room, User.get(movee));
            sendLine('You was moved to another room by ' + myNick, movee);
        } else {
            sendLine('/move needs a room and a name');
            return;
        }
    // check alias
        ...
        if (!User.has(checked)) {
            sendLine('There is no user named "' + checked + '"');
            return;
        }
        var IP = User.get(checked).conn.remoteAddress;
        // Find aliases
        var aliasCount = 0;
        sendLine('The user with the IP ' + IP + ' has these alias:');
        User.forEach(function (iterUser) {
            if (iterUser.conn.remoteAddress === IP) {
        ...
        sendLine('(' + aliasCount + ' total alias)');
    // broadcast message
        ...
        console.log('Menssage: "' + broadcast + '" from user "' + myNick + '"');
    ...
    // change bits
        ...
        if (pos !== -1) {
            ...
            if (!User.has(receiver)) {
                sendLine('There is no user named "' + receiver + '"');
                return;
            }
            if (User.hasBits(receiver) === null) {
                sendLine('The user "' + receiver + '" does not have a account.');
                return;
            }
            amount = parseInt(amount);
            if (Number.isNaN(amount) || !Number.isFinite(amount)) {
                sendLine('Invalid amount.');
                return;
            }
            var oldBalance = User.hasBits(receiver);
            if (User.changeBits(receiver, amount)) {
                sendLine('You gave "' + amount + '" bits to' + receiver + ' bits ');
                sendLine('You got bits! ' + amount + ' from "' + user.nick + '"', receiver);
                modLogger.logBitsChange(myNick, receiver, amount, oldBalance, User.hasBits(receiver), User.get(receiver).obj);
            } else {
                sendLine("Error adding bits");
            }
        } else {
            sendLine('/bits needs an amount and a username.');
            return;
        }
    // moderation log
        ...
        switch (msg.type) {
                ...
                if (!User.hasAccount(myNick)) {
                    User.assert(msg.assertion, function (good, email) {
                        if (good) {
                            ...
                            user.send({
                                type: 'console_msg',
                                msg: 'Something went wrong with login.'
                            });
                        }
                    });
                } else {
                ...
                if (msg.name.indexOf(' ') === -1) {
                    ...
                    if (msg.name.substr(0, 6) === 'house ') {
                        ...
                        if (User.hasAccount(houseName)) {
                            ...
                            user.send({
                                type: 'console_msg',
                                msg: 'The user with nick: "' + houseName + '" is homeless.'
                            });
                        }
                    } else {
                        user.kick('protocol_error');
            ...
            case 'change_house_background':
                if (User.hasAccount(myNick)) {
                    ...
                    if (msg.bg_name === null) {
                        house.background = {
                            data: '/gemplace/media/rooms/cave.png',
                            width: 960,
                            height: 660,
                            iframe: false
                        };
            ...
            if (User.getUserData(nick, 'last_reward', '1970-01-01') !== date) {
                if (User.hasBits(nick) < 500) {
                    ...
                    if (User.changeBits(nick, reward)) {
                        ...
                        user.send({
                            type: 'console_msg',
                            msg: "As a thanks for visiting gemplace again today, here's " + reward + " free bits! :)"
                        });
                    } else {
                        user.send({
                            type: 'console_msg',
                    ...
                    user.send({
                        type: 'console_msg',
                        msg: ">tfw you have more than 420000 bits"
                    });
                }
            }
        }
